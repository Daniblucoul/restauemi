...
router.post('/', (req, res) => {
  const { first_name, last_name, role, phone, email, hire_date } = req.body;
  
  const query = `INSERT INTO staff (first_name, last_name, role, phone, email, hire_date) 
                 VALUES (?, ?, ?, ?, ?, ?)`;
  
  db.run(query, [first_name, last_name, role, phone, email, hire_date], function(err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    res.status(201).json({
...
router.put('/:id', (req, res) => {
  const { first_name, last_name, role, phone, email, hire_date, status } = req.body;
  
  const query = `UPDATE staff 
                 SET first_name = ?, last_name = ?, role = ?, phone = ?, email = ?, hire_date = ?, status = ?
                 WHERE id = ?`;
  
  db.run(query, [first_name, last_name, role, phone, email, hire_date, status, req.params.id], function(err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
